...
export async function dataCalculator() {
    ...
    const projectWorkXP = (await getProjectWork_xp_view()) as receivedXPFromAirtableType[];
    const indieWorkXP = (await getIndieWork_xp_view()) as receivedXPFromAirtableType[];
    console.log('indieWorkXP', indieWorkXP);
    const internalOpsXP = (await getInternalOperations_xp_view()) as receivedXPFromAirtableType[];
    const workingGroupsXP = (await getWorkingGroups_xp_view()) as receivedXPFromAirtableType[];
    ...
    const projectWorkCalculatedXP: xpType[] = sourceTotalXPCalculator(projectWorkXP).map((person) => {
        return {
            name: person.name,
    ...
    const indieWorkCalculatedXP: xpType[] = sourceTotalXPCalculator(indieWorkXP).map((person) => {
        return {
            name: person.name,
            skills: xpSumBySkillCalculator(person),
            total_amount: xpSumPerPersonCalculator(person).total_amount,
    ...
    const internalOpsCalculatedXP: xpType[] = sourceTotalXPCalculator(internalOpsXP).map((person) => {
        return {
            name: person.name,
            skills: xpSumBySkillCalculator(person),
            total_amount: xpSumPerPersonCalculator(person).total_amount,
    ...
    const workingGroupsCalculateXP: xpType[] = sourceTotalXPCalculator(workingGroupsXP).map((person) => {
        return {
            name: person.name,
            skills: xpSumBySkillCalculator(person),
            total_amount: xpSumPerPersonCalculator(person).total_amount,
    ...
    const bountiesCalculatedXP: xpType[] = sourceTotalXPCalculator(bountiesXP).map((person) => {
        return {
            name: person.name,
            skills: xpSumBySkillCalculator(person),
            total_amount: xpSumPerPersonCalculator(person).total_amount,
    ...
    const stackExchangeCalculatedXP: xpType[] = sourceTotalXPCalculator(stackExchangeXP).map((person) => {
        return {
            name: person.name,
            skills: xpSumBySkillCalculator(person),
            total_amount: xpSumPerPersonCalculator(person).total_amount,
    ...
    stackExchangeCalculatedXP.forEach((person) => {
        if (!personDetailsData.some((personDetails) => personDetails.name === person.name)) {
    ...
    const personData: dashboardDataType[] = personDetailsData.map((person) => {
        const xpSourcesSum = overallXPDetails(
            [
                ...
                internalOpsCalculatedXP.find((personXP) => personXP.name === person.name),
                workingGroupsCalculateXP.find((personXP) => personXP.name === person.name),
        ...
        return {
            ...
            projectWorkXP: projectWorkCalculatedXP.find((personXP) => personXP.name === person.name),
            indieWorkXP: indieWorkCalculatedXP.find((personXP) => personXP.name === person.name),
            internalOpsXP: internalOpsCalculatedXP.find((personXP) => {
                return personXP.name === person.name;
            }),
            workingGroupsXP: workingGroupsCalculateXP.find((personXP) => personXP.name === person.name),
            bountiesXP: bountiesCalculatedXP.find((personXP) => personXP.name === person.name),
            stackExchangeXP: stackExchangeCalculatedXP.find((personXP) => personXP.name === person.name),
        };
    });
